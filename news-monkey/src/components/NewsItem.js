      ...
      <div className="my-3">
        <div className={`card ${cardClass} h-100 shadow-sm border-0`} style={{borderRadius: '15px', overflow: 'hidden', transition: 'all 0.3s ease', backgroundColor: theme ? theme.cardBg : '#ffffff'}}>
            ...
            <div style={{position: 'relative', overflow: 'hidden'}}>
              <img src={imageUrl ? imageUrl : "/images/default-news.png"} className="card-img-top" alt={title} style={{height: "220px", objectFit: "cover", transition: 'transform 0.3s ease'}} onMouseOver={e => e.target.style.transform = 'scale(1.05)'} onMouseOut={e => e.target.style.transform = 'scale(1)'}/>
            ...
            <div className="card-body" style={{padding: '1.5rem'}}>
                ...
                <p className="card-text mb-3"><small className={`d-flex align-items-center ${theme && theme.isDarkMode ? 'text-light' : 'text-muted'}`} style={{fontSize: '0.8rem'}}>
                ...
                <div className="d-flex justify-content-between align-items-center pt-2" style={{borderTop: theme && theme.isDarkMode ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.1)'}}>
                  <a href={newsUrl} target="_blank" rel="noreferrer" className={`btn btn-sm ${theme && theme.isDarkMode ? 'btn-light' : 'btn-dark'} px-3 py-2`} style={{borderRadius: '25px', fontWeight: '500', fontSize: '0.85rem', textDecoration: 'none'}}>Read More</a>
                  <div className="d-flex gap-2">
                    ...
                    <button 
                      className={`bookmark-btn ${this.state.isBookmarked ? 'text-danger' : (theme && theme.isDarkMode ? 'text-light' : 'text-muted')}`}
                      onClick={this.toggleBookmark}
                      title={this.state.isBookmarked ? 'Remove from bookmarks' : 'Add to bookmarks'}
                      style={{
                        padding: '8px 12px',
                        borderRadius: '50%',
                        boxShadow: this.state.isBookmarked ? '0 2px 8px rgba(220, 53, 69, 0.3)' : 'none'
                      }}
                    >
